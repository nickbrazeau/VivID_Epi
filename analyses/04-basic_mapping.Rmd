...
jpeg("~/Documents/GitHub/VivID_Epi/results/figures/Cluster_Prevalence_3x1.jpg",
     heigh = 8, width = 11, units = "in", res=500)
cowplot::plot_grid(mp$plots[[3]],  mp$plots[[4]],  mp$plots[[6]],
                   align = "h",
                   labels = c("A", "B", "C"))
...
# Plot Cases
#......................
case_n_maps <- purrr::pmap(list(data = mp$data, plsmdmspec = mp$plsmdmspec), casemap_n_plotter)
...
caseprevmaps <- purrr::pmap(list(data = mp$data, plsmdmspec = mp$plsmdmspec), casemap_prev_plotter)
...
#........................
gcdist <- mp %>% 
  dplyr::filter(maplvl == "hv001" & plsmdmspec == "pfldh") %>% # just getting long lat, so species doesn't matter 
  tidyr::unnest() %>% 
...
moranIclst$Istatistic <- purrr::map(moranIclst$moranIclst, "statistic")
moranIclst$pvalue <- purrr::map(moranIclst$moranIclst, "p.value")

moranIclst <- moranIclst %>% 
  dplyr::select(-c("moranIclst")) %>% 
  tidyr::unnest(cols = c("Istatistic", "pvalue"))
...
ape.range.plotObj <- ggplot() + 
  ...
  geom_vline(xintercept = ape.range.realdata, color = "red", size=2) + 
...
chimpgor.range.iters$chimpgor_overlap_prev <- purrr::map(chimpgor.range.iters$iter, function(x){
  return( run_chimpgor_permutations(dt18s = pv18s.dt, n.chimpgorclsts = length(chimpgor.clusters)) )
}) 
...
chimpgor.range.plotObj <- ggplot() + 
  ...
  geom_vline(xintercept = chimpgor.range.realdata, color = "red", size=2) + 
