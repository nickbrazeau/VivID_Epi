...
#### Positive Cluster Level Prevalence 
```{r, results='asis', fig.align='center', fig.width=11, fig.height=8}
...
clst.prev.plotObj <- clst %>% 
  ...
  dplyr::select(c("adm1name", "hv001", "longnum", "pv18sprev", "pv18sL95", "pv18sU95")) %>%
  dplyr::filter(pv18sprev != 0) %>% 
  tidyr::gather(., key = "estlevel", value = "est", 4:ncol(.)) %>% 
  dplyr::mutate(estlevel = gsub("pv18s", "", estlevel)) %>% 
  tidyr::spread(., key = "estlevel", value = "est", 5:6) %>%
  ...
  ggplot() + 
  geom_pointrange(aes(x = hv001, y=prev, ymin=L95, ymax=U95,
                      color = adm1name), 
                  size =0.5, alpha = 0.5) +
...
dir.create(path = "~/Documents/GitHub/VivID_Epi/results/figures/", recursive = T)
jpeg(filename = "~/Documents/GitHub/VivID_Epi/results/figures/pos_cluster_prevalence_pointranges_E-W_colorgrad.jpg",
     width = 11, height = 8, units = "in", res=500)
plot(clst.prev.plotObj)
graphics.off()
...
Not considering weights, among the `r nrow(clst)` clusters considered, `r sum(clst$pv18sn >= 1)` clusters had at least one infection and `r sum(clst$pv18sn == 1)` had exactly one infection. 
