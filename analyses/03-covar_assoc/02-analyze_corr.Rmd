...
dt.cc <- readRDS("~/Documents/GitHub/VivID_Epi/data/derived_data/vividepi_recode_completecases.rds")
...
```{r}
covar.files <- list.files("~/Documents/GitHub/VivID_Epi/analyses/03-covar_assoc/_rslurm_covar_corr/", pattern = ".RDS", full.names = T)
...
read_covar_files <- function(path){
  ret <- readRDS(path)
  ret <- unname(ret)
  return(ret)
}
...
covar.results.vals <- lapply(covar.files$covar.files, read_covar_files) %>% 
  do.call("rbind", .)


paramsdf <- readRDS("~/Documents/GitHub/VivID_Epi/analyses/03-covar_assoc/_rslurm_covar_corr/params.RDS") %>% 
  dplyr::select(-c(data))

covar.results <- paramsdf
covar.results$dist <- unlist(covar.results.vals)
