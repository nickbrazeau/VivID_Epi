---
title: "Bayesian Hierarchial Spatial Province Models for _P. vivax_ among Asymptomatic Adults in the DRC"
...
mydiagnosticdir <- "analyses/08-spatial_prediction/prov_map_diagnostic_chains/"
dir.create(mydiagnosticdir, recursive = T)
wrap_chain_plotter(diag.dir = mydiagnosticdir, chains = chains)
...
mod.framework.long$n.sample <- 1e6 + 1e3
...
mod.framework.long$summresults <- purrr::map(mod.framework.long$summresults, function(x){
  pars <- rownames(x)
  ret <- cbind.data.frame(pars = pars, as.data.frame(x))
  return(ret)
})
...
saveRDS(objetct = mod.framework.long, 
        file = "~/Documents/GitHub/VivID_Epi/results/Province_Bayes_Hierarchial_Model_longchain.rds")
...
# Let's Look at the Chains
#............................
mod.framework.long$samples <- purrr::map(mod.framework$MCMC, "samples")
mod.framework.long$beta <- purrr::map(mod.framework$samples, "beta")
mod.framework.long$phi <- purrr::map(mod.framework$samples, "phi")
mod.framework.long$tau2 <- purrr::map(mod.framework$samples, "tau2")
mod.framework.long$rho <- purrr::map(mod.framework$samples, "rho")
mod.framework.long$fitted <- purrr::map(mod.framework$samples, "fitted")
...
myfinalchain <- "analyses/08-spatial_prediction/prov_map_final_long_chain/"
dir.create(myfinalchain, recursive = T)
wrap_chain_plotter(diag.dir = myfinalchain, chains = chains)
