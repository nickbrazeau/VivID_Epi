...
##### Good Priors for Intercept Matern function 
```{r, results='asis'}
ests <- summary(prevmap.linear.mleintercept)$cov.pars
ests <- exp(ests)
ests <- cbind.data.frame(covparam = c("sigma2", "phi", "tau2"), ests)
ests %>% 
...
prevmap.linear.mlecovar <- PrevMap::linear.model.MLE(
  formula=as.formula("plsmdn~precip_mean_cont_scale_clst+cropprop_cont_scale_clst+hlthdist_cont_scale_clst"), 
  coords=as.formula("~longnum+latnum"),
  data=pvclust.weighted.nosf, 
  start.cov.pars=c(1,1),
  kappa=0.75)
...
##### Good Priors for Covar Matern function 
```{r, results='asis'}
