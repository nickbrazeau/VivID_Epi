...
prevmap.linear.mlecovar <- PrevMap::linear.model.MLE(
  formula=as.formula("plsmdn~precip_mean_cont_scale_clst+cropprop_cont_scale_clst+frctmean_cont_scale_clst"), 
  coords=as.formula("~longnum+latnum"),
  data=pvclust.weighted.nosf, 
  start.cov.pars=c(1,1),
  kappa=0.75)
...
mod.framework <- readRDS("analyses/07-spatial_prediction/_rslurm_prevmap_diagnostic_chains/params.RDS")

mcmc.ret <- list.files(path = "analyses/07-spatial_prediction/_rslurm_prevmap_diagnostic_chains/", full.names = T, pattern = ".RDS")
...
long.intercept <- readRDS("analyses/07-spatial_prediction/prevmap_long_chains/longrun-prevmapbayes-intercept.rds")
long.covar <- readRDS("analyses/07-spatial_prediction/prevmap_long_chains/longrun-prevmapbayes-mod.rds")
