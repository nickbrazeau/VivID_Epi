...
long.framework$DIC <- purrr::map(long.framework$LL, function(x){
  mu = mean(x)
  sigma = var(x)/2
  DICg = mu + sigma
  return(DICg)
...
long.framework %>% 
  ...
  tidyr::unnest(cols="DIC") %>% 
  magrittr::set_colnames(c("Model", "DIC")) %>% 
  kableExtra::kable(format = "html", escape = F) %>%
  kableExtra::kable_styling("striped", full_width = F) 
...
plot(long.framework$LL[[1]], main = "Intercept Model")
plot(long.framework$LL[[2]], main = "Covars Model")
