...
provmodel.posteriorselotObj <- dplyr::left_join(DRCprov, provmodel.posteriorse) %>%  
  geom_sf(data = ., aes(fill = posteriorses), color = "#737373") +
...
# no smoothing like in "basic eda plots"
ret.smrstr.m  <-  raster::rasterToPoints(clstmodel.fitted.postmeans.raster)
...
ret.smrstr.m.plot <- ggplot() + 
  ...
  theme(legend.position = "left",
        legend.text = element_text(size = 14, face = "bold", angle = 0),
        legend.text = element_text(size = 15, face = "bold", vjust = 0.5, hjust = 0.5))
