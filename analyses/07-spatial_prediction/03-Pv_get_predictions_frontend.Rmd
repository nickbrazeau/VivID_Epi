...
drccites <- readr::read_csv("data/map_bases/DRC_city_coordinates.csv") %>% 
  dplyr::filter(population > 1e6 | city %in% c("Kisangani", "Goma"))
drccites <- drccites %>% 
...
provmodel.samples.fittedvalues_prevs <- apply(provmodel.samples.fittedvalues, 
                                              2, function(x){x/provmodel$data[[1]]$n})
provmodel.se <- apply(provmodel.samples.fittedvalues_prevs, 2, sd)
...
summprevpe <- summary(clstmodel.fitted.sedf$PrevSE)
knitr::kable(tibble::tibble(nms = names(summprevpe), ret = unname(unlist(summprevpe))))
...
crop.plot <- ggplot() + 
  ggspatial::layer_spatial(data = covar.rstr.pred[["cropprop_cont_scale_clst"]], 
                                      aes(fill = stat(band1))) +
  scale_fill_viridis_c("Crop Prop.", na.value = NA) +
  theme_void()
...
cowplot::plot_grid(precip.plot, crop.plot, hlthdist.plot,
                   align = "v", ncol = 1)
graphics.off()


svglite::svglite(file = "results/figures/Clst_Covariates.svg",
                 height = 11, width = 8)
cowplot::plot_grid(precip.plot, crop.plot, hlthdist.plot,
