...
```{r, results='asis'}
provmodel.posteriormeans <- cbind.data.frame(adm1name = provs, posteriormeans = provmodel.posteriormeans) 
...
provmodelplotObj <- ggplot() + 
  ...
  geom_sf(data = provmodelplotObjdf, aes(fill = posteriormeans), alpha = 0.7, color = "#737373") +
...
clstmodel.rstr.smooth <- raster::focal(clstmodel.fitted.postmeans.raster, w=matrix(1,
                                                                                   nrow=5,
                                                                                   ncol=5), mean)
