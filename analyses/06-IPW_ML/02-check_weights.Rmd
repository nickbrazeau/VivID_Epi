...
```{r, results='asis'}
cbind.data.frame(params$label, widist) %>% 
  DT::datatable(., 
                rownames = F,
                options = list(
                  searching = F,
...
iptw.dist.plotObj <- params %>% 
  dplyr::select(c("label", "iptw")) %>% 
  tidyr::unnest(cols = iptw) %>% 
  dplyr::mutate(logiptw = log(iptw)) %>% 
  ggplot() + 
  geom_boxplot(aes(x = label, y = logiptw)) +
...
# make pretty plot
varnames <- params %>% 
  dplyr::select(c("target", "label")) %>% 
  dplyr::rename(covar1 = target)

covarbalanceplot <- covarbalparams %>% 
  dplyr::left_join(., varnames, by = "covar1") %>% 
  dplyr::mutate(lvl_fct = factor(lvl, levels = c("base", "IPTW"), labels = c("Base", "IPW"))) %>% 
  ggplot() + 
  geom_boxplot(aes(x = label, y = dist)) +
