...
blocks <- data.frame(ymin = c(0, 0, 0),
                     ymax = c(3.5, 3.5, 3.5),
                     xmin = c(0.5, 6.5, 12.5), 
                     xmax = c(2.5, 10.5, 14.5)
)

text <- data.frame(x = c(1.5, 4.75, 8.5, 11.5, 13.5),
...
### Appendix: Raster Digging into _Pv_-Farming/Crop-Land 
```{r}
# load spatial coordinates
ge <- readRDS("data/raw_data/dhsdata/VivIDge.RDS")
...
cropraster <- raster::aggregate(cropraster, fact = 20) # agg for easier plotting 
...
pvcropplot <- pvsrvyr %>% 
  ...
  geom_point(aes(x = longnum, y = latnum, color = pv, size = n),
             alpha = 0.6) +
  scale_fill_viridis_c("crop", na.value = NA) +
  scale_color_distiller("Pv Prev", type = "div", palette = "RdYlBu",
                       na.value = NA) +
  theme_void() +
  theme(legend.position = "bottom")

jpeg(filename = "~/Documents/GitHub/VivID_Epi/results/figures/pvcrop_raster_figure.jpg", width = 11, height = 8, units = "in", res = 600)
plot(pvcropplot)
graphics.off()
...
hlthdistraster <- readRDS("data/derived_data/vividep_hlthdist_wlktime_raster.rds")
