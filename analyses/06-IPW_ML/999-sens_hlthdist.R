...
dt <- dt  %>% 
  dplyr::select(c("pv18s", "pfldh", "po18s", dcdr, "hlthdist_cont_log_scale_clst")) %>% 
...
dtsrvy_raw <- dt %>% srvyr::as_survey_design(ids = hv001, 
                                             strata = hv023, 
                                             weights = hiv05_wi)
ret <- survey::svyglm("pv18s ~ hlthdist_cont_log_scale_clst",
                      design = dtsrvy_raw,
                      family = quasibinomial(link="logit"))
broom::tidy(ret, exponentiate = T, conf.int = T)


ret <- survey::svyglm("pfldh ~ hlthdist_cont_log_scale_clst",
                      design = dtsrvy_raw,
                      family = quasibinomial(link="logit"))
broom::tidy(ret, exponentiate = T, conf.int = T)
...
txs <- txs %>% 
  dplyr::filter(target == "hlthdist_fctb_clst") %>% 
  dplyr::mutate(target = "hlthdist_cont_log_scale_clst",
                var_label = "Cont Health Dist",
                abridged_var_label = "Cont Health Dist",
                type = "continuous")
