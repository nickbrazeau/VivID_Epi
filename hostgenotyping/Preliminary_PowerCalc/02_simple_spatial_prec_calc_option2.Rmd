...
##            Read-In Data          ## 
##----------------------------------##
load("~/Documents/GitHub/VivID_PhD_Thesis/HostGenotyping/Preliminary_PowerCalc/data.Rda")
...
conditionalPanel(
      ...
      sliderInput("duffypAF", label = "Mutatation Frequency (Prior)",
              min = 0.8, max = 1, value = 0.9, step = 0.01)
   )
...
dhs13_sub_sum <- dhs13_sub_sum %>% 
  ...
  dplyr::mutate(CLwidth = wtU95CI - wtL95CI) %>% 
...
plotObj <- ggplot() + 
  geom_sf(data=dhs13_mapdf, aes(fill = CLwidth)) +
 scale_fill_gradientn("Mutation \n CI Width", colours = c("#d0d1e6", "#3690c0", "#253494")) +
