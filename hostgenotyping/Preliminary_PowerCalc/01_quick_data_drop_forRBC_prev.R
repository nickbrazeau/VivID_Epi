...
##           Shape Objects          ## 
##----------------------------------##
DRCprovfiles <- dir("~/Google_Drive/PhD_Work/Maps/DRC_GADM/sf_gadm/", full.names = T)
DRC <- lapply(DRCprovfiles, function(x)readRDS(x))
DRCprov <- DRC[[2]]

provdict <- read_tsv(file = "~/Google_Drive/PhD_Work/Maps/DRC_GADM/dhs_shnprovin_mapfile_variabledocumentation.tab.txt")
...
save(dhs13_short, DRCprov, provdict, prelim, file = "~/Documents/GitHub/VivID_PhD_Thesis/HostGenotyping/Preliminary_PowerCalc/data.Rda")
