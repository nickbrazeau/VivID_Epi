...
# Import Data
#......................
source("~/Documents/GitHub/VivID_Epi/analyses/04-basic_mapping.R")
...
### Unstandardized Point Estimate Prevalence Maps by Species {.tabset .tabset-fade .tabset-pill}
...
### Standardized Point Estimate Prevalence Maps by Species {.tabset .tabset-fade .tabset-pill}
Each tab is a prevalence map for a malaria species
...
dt %>% 
  ...
  ggplot() +
  geom_violin(aes(x=factor(plsmdn), y=plsmd, colour = factor(plsmdn))) +
  vivid_theme
...
plotObj <- dplyr::bind_cols(poprov, pvprov) %>% 
  ggplot(aes(x=Po_prev, y=Pv_prev, size = plsmdn)) +
  geom_point(aes(colour = adm1name)) +
  geom_smooth(method="loess", se=F, colour = "red") +
...
#  dplyr::filter(!(Po_prev == 0 & Pv_prev == 0)) %>% 
  ...
  geom_point(aes(colour = hv001)) +
  geom_smooth(method="loess", se=F, colour = "red") +
      ggtitle("Pfalciparum versus Povale Province Prevalence") + ylab("Pv Prevalence") + xlab("Po Prevalence") +
    vivid_theme
