...
All continuous variables are scaled for parametric estimates.   Signficant variables are highlighted in blue and all cluster-level covariates are italicized. 
...
pvivriskfactortable <- pvivriskfactortable %>% 
  ...
  dplyr::select(c("Covariates", "levels", "Pvivax-Negative", "Pvivax-Positive", "estimate", "conf.low", "conf.high"))
...
pfalriskfactortable <- pfalriskfactortable %>% 
  ...
  dplyr::select(c("Covariates", "levels", "Pfalciparum-Negative", "Pfalciparum-Positive", "estimate", "conf.low", "conf.high"))
...
morani <- readRDS("~/Documents/GitHub/VivID_Epi/results/basic_map_moranI.rds")
```
