...
##            Read-In Data          ## 
##----------------------------------##
load("~/Documents/GitHub/VivID_Epi/hostgenotyping/Preliminary_PowerCalc/data.Rda")
...
$P(S=1 \mid C = 0) = binom(n,p)$, where $p$ is equal to the sampling weights that an individual was selected into the DHS (to perserve the two-stage stratified random sampling strucutre).   
...
conditionalPanel(
      ...
      sliderInput("duffypAF", label = "Mutatation Frequency (Prior)",
              min = 0.8, max = 1, value = 0.9, step = 0.05)
   )
...
conditionalPanel(
      ...
      sliderInput("hbspAF", label = "Mutatation Frequency (Prior)",
              min = 0.08, max = 0.2, value = 0.1, step = 0.05)
   )
      ...
      sliderInput("hbcpAF", label = "Mutatation Frequency (Prior)",
              min = 0, max = 0.025, value = 0.01, step = 0.005)
   )
...
design <- survey::svydesign(ids=dhs13_sub$barcode,
                              weights=dhs13_sub$sampleweight,
                              data=dhs13_sub)
